...
class DiscretizedMixturelogisticLoss(nn.Module):
    ...
    def forward(self, input, target, lengths=None, mask=None, max_len=None):
        ...
        losses = discretized_mix_gaussian_loss(
            input, target, num_classes=hparams.quantize_channels,
            log_scale_min=hparams.log_scale_min, reduce=False)
        assert losses.size() == target.size()
        return ((losses * mask_).sum()) / mask_.sum()
class DiscretizedGaussianLoss(nn.Module):
    def __init__(self):
        super(DiscretizedGaussianLoss, self).__init__()
...
def save_waveplot(path, y_hat, y_target, writer=None,step=0):
    ...
    if writer:
        ...
        writer.add_image('image', im,global_step=step)
    plt.close()
...
def save_states(global_step, writer, y_hat, y, input_lengths, checkpoint_dir=None):
    ...
    save_waveplot(None, y_hat, y, writer,global_step)
...
def train_loop(model, data_loaders, optimizer, writer, checkpoint_dir=None, scheduler=None):
    if use_cuda:
        model = model.cuda()
...
def save_checkpoint(model, optimizer, step, checkpoint_dir, epoch, ema=None, scheduler=None):
...
def load_checkpoint(path, model, optimizer, reset_optimizer, scheduler=None):
    global global_step
    global global_epoch
    global global_test_step
...
if __name__ == "__main__":
    ...
    if checkpoint_path is not None:
        load_checkpoint(checkpoint_path, model, optimizer, reset_optimizer, scheduler)

    # Setup summary writer for tensorboard
    if log_event_path is None:
        log_event_path = "log/teacher"
    print("TensorBoard event log path: {}".format(log_event_path))
    writer = SummaryWriter(log_dir=log_event_path)

    # Train!
    try:
        train_loop(model, data_loaders, optimizer, writer, checkpoint_dir=checkpoint_dir, scheduler=scheduler)
    except KeyboardInterrupt:
        print("Interrupted!")
        pass
    finally:
        save_checkpoint(
            model, optimizer, global_step, checkpoint_dir, global_epoch, scheduler=scheduler)
